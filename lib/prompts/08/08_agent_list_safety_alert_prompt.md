## Prompt: Gerar listagem de alertas de segurança (safety_alerts)

Parâmetros (defina estes valores no início do prompt antes de executar)
---------------------------------------------------------------
- ENTITY_SINGULAR: Alerta de segurança   # nome singular legível (ex.: 'Alerta de segurança')
- ENTITY_PLURAL: safety_alerts           # nome plural legível em minúsculas (ex.: 'safety_alerts')
- DTO_CLASS: SafetyAlertDto             # nome da classe/DTO usada no projeto (ex.: 'SafetyAlertDto')
- FEATURE_FOLDER: safety_alerts         # pasta/feature onde o código vive (ex.: 'safety_alerts')
...
- SORT_BY_DEFAULT: reportedAt           # campo de ordenação padrão
- INCLUDE_HINT: locations,reporter     # relacionamentos opcionais sugeridos

OBS: Substitua os tokens acima antes de enviar o prompt ao agente ou forneça esses valores na invocação.

Contexto
---
Você é um agente que ajuda a produzir tanto a especificação de dados quanto, quando solicitado, código Flutter/Dart que implemente a listagem de alertas de segurança (widget/fluxo) seguindo as convenções do repositório. O prompt abaixo está parametrizado para permitir gerar:

- uma especificação/contract (JSON) para a API/DAO que fornece a listagem;
- e opcionalmente, um widget Flutter/Dart muito semelhante ao `lib/features/safety_alerts/presentation/safety_alerts_page.dart`, com integração ao DAO local da feature.
...
2. Código Flutter/Dart que implemente a página/listagem de alertas de segurança (widget), integrando com o DAO local da feature, e obedecendo às convenções do projeto (idioma, nomes, patterns de persistência e handlers de UI).

Entradas esperadas (inputs)
---
- filters: objeto opcional com critérios de busca (por exemplo: {"q": "assalto", "severity": "high"})
- page: número da página (inteiro, default 1)
- pageSize: itens por página (inteiro, default 20)
- sortBy: campo para ordenação (por exemplo: "reportedAt", "severity")
- sortDir: direção da ordenação ("asc" ou "desc")
- include: lista opcional de relacionamentos a incluir (por exemplo: ["locations","reporter"]) — o agente deve explicar resoluções possíveis.

Se o modo de execução pedir geração de código, o agente deve aceitar parâmetros adicionais (passe-os como tokens no topo):

- DAO_CLASS_HINT: SafetyAlertsLocalDaoSharedPrefs  # sugestão de nome do DAO local a ser importado
...
- Campos sensíveis devem ser mascarados na saída de exemplo (por exemplo: IDs de usuário/denunciantes parcialmente ocultos) e, no código gerado, aplicar máscaras quando exibir tais campos.
...
- Permissões: o código gerado e a especificação devem mencionar que a listagem respeita permissões/escopo do usuário (exibir apenas alertas que o usuário tem acesso).
...
- Exibição de `severity`, `reportedAt` (formatado) e `location` quando presentes.
- Integração com DAO local da feature: identificar e importar o DAO (por exemplo `SafetyAlertsLocalDaoSharedPrefs`) e usar os métodos de leitura (`listAll`) — operações de escrita/remoção/edição devem ser mantidas fora deste arquivo e implementadas separadamente.
...
---
O agente deve mapear o DTO aos campos usados no código existente. Campos típicos para um alerta de segurança:

- id: string
- title: string
- description: string (opcional)
- severity: string (ex: "low","medium","high")
- location: { lat: number, lon: number, address?: string } ou string simplificada
- image_url: string (opcional)
- reportedBy: string (mascarado quando exibido)
- reportedAt: ISO8601 datetime
- status: string (ex: "open","investigating","resolved")
- related_incidents: array opcional
...
- data: lista de objetos de alerta, cada um contendo ao menos os campos abaixo

Campos de cada alerta (exemplo mínimo)
---
- id: string (UUID ou identificador)
- title: string
- description: string opcional
- severity: string (ex: "high")
- location: objeto ou string (ex: {"lat": -23.58, "lon": -46.66, "address": "Av. Paulista"})
- image_url: string (URL da imagem) — opcional
- reportedBy: string (mascarado, ex: "usr_45***21")
- reportedAt: ISO8601 datetime
...
```json
{
  ...
  "data": [
    {
      "id": "d9f8e7c6-b5a4-3210-ffff-111222333444",
      "title": "Assalto em via pública",
      "description": "Relato de assalto próximo ao metrô.",
      "severity": "high",
      "location": { "lat": -23.561, "lon": -46.655, "address": "R. Haddock Lobo, 100" },
      "image_url": "https://cdn.example.com/alerts/alert1.jpg",
      "reportedBy": "usr_45***21",
      "reportedAt": "2025-11-01T20:15:00Z",
      "status": "open",
      "createdAt": "2025-11-01T20:16:00Z",
      "updatedAt": "2025-11-01T20:20:00Z"
    }
  ]
}
```
...
1. O agente gera um widget Flutter/Dart que implementa a página de listagem com comportamento compatível com `safety_alerts_page.dart` (loading, list, image handling).
...
- Sugira campos opcionais que podem ser incluídos se solicitado (ex.: tags, last_seen_at) e como isso impacta performance.
...
- adicione apenas os arquivos mínimos necessários (ex.: `safety_alerts_page.dart`, `safety_alert_list_widget.dart`, `safety_alert_list_item.dart`) e siga as convenções de nomes e pastas;
...
Resumo
---
Este prompt agora está parametrizável e suporta dois modos: gerar especificação (JSON/contract) ou gerar implementação Flutter/Dart de uma página de listagem compatível com `safety_alerts_page.dart`. Configure os tokens no topo (`ENTITY_SINGULAR`, `DTO_CLASS`, `FEATURE_FOLDER`, `DAO_CLASS_HINT`, etc.) antes de executar o prompt com o agente.
